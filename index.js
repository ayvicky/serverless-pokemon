...
app.get('/pokemon/:id', (req, res) => {
    ...
    const query = `SELECT * FROM pokemon_tbl WHERE id = ${id}`
    pool.query(query, (err, results, fields) => {
        if(err) {
            const response = { data: null, message: err.message,}
            res.send(response)
...
app.post('/pokemon', (req, res) => {
    ...
    const query = `INSERT INTO pokemon_tbl (name, height, weight, avatar) VALUES ('${name}', ''${height}', '${weight}', '${avatar}')`
    pool.query(query, (err, results, fields) => {
        if(err) {
            const response = { data: null, message: err.message,}
            res.send(response);
...
app.put('/pokemon/:id', (req, res) => {
    const { id } = req.params;
    const query = `SELECT * FROM pokemon_tbl WHERE id = ${id} LIMIT 1`
    pool.query(query, (err, results, fields) => {
        if(err) {
            const response = { data: null, message: err.message, }
            res.send(response);
        ...
        const query = `UPDATE pokemon_tbl SET name='${name}', height='${height}', weight='${weight}', avatar='${avatar}' WHERE id='${id}'`
        pool.query(query, (err, results, fields) => {
            if(err) {
                const response = { data: null, message: err.message, }
                res.send(response)
...
// Handle pokemon DELETE route
app.delete('/pokemon/:id', (req, res) => {
    const { id } = req.params
    const query = `DELETE FROM pokemon_tbl WHERE id=${id}`
    pool.query(query, (err, results, response) => {
        if(err){
            const response = { data: null, message: err.message}
            res.send(response);
        }
